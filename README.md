...
- 仅适用于 Windows。
- 所有文件存放于 `$env:USERPROFILE\Apps\sing-box_with_xray`, 如有需要请自行更改 `$WorkDir`。
- 自行更改 `xray.json` 中的参数, 更多模板参考 `templates` 目录。
- 确保 `.ps1` 脚本文件的编码为 `UTF-8 with BOM`, `CRLF`。

## 初次使用

1. 将 `Restart.ps1`、`Stop.ps1`、`Update.ps1`、`sing-box.json` 和修改好的 `xray.json` 放入 `$env:USERPROFILE\Apps\sing-box_with_xray`。
...
```powershell
powershell.exe -ExecutionPolicy Bypass -File "C:\Users\admin\Apps\sing-box_with_xray\Stop.ps1"
```
